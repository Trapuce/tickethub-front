...
export default function EventList() {
    ...
    const normalizeDate = (date: number) => (date < 1e12 ? date * 1000 : date);
      ...
      .filter((event) => normalizeDate(event.eventDate) > Date.now())
      .sort((a, b) => normalizeDate(a.eventDate) - normalizeDate(b.eventDate));
  


        const pastEvents = events
        .filter((event) => normalizeDate(event.eventDate) <= Date.now())
        .sort((a, b) => normalizeDate(b.eventDate) - normalizeDate(a.eventDate));

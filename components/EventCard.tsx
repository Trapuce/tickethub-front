...
import { CalendarDays, Ticket, StarIcon, PencilIcon, Check, XCircle } from 'lucide-react';
...
export default function EventCard({ eventId }: { eventId: Id<'events'> }) {
  ...
  // Fetch necessary data using Convex queries
  ...
  const isPastEvent = normalizeDate(event.eventDate) < Date.now();
  ...
  const renderTicketStatus = () => {
    ...
    if (queuePosition) {
      return (
        <div className="mt-4">
          {queuePosition.status === 'offered' && <PurchasedTicket eventId={eventId} />}
          {renderQueuePosition()}
          {queuePosition.status === 'expired' && (
            <div className="p-3 bg-red-50 rounded-lg border border-red-100">
              <span className="flex items-center text-red-700 font-medium">
                <XCircle className="w-5 h-5 mr-2" /> Offer expired
              </span>
            </div>
          )}
        </div>
      ...
      <div className={`p-6 ${imageUrl ? 'relative' : ''}`}>
        <div className="flex justify-between items-start">
          <div className="flex flex-col items-start gap-2">
            {isEventOwner && (
              <span className="inline-flex items-center gap-1 bg-blue-600/90 text-white px-2 py-1 text-xs font-medium">
                Your Event
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center text-gray-600">
            ...
            <span>
              {new Date(normalizeDate(event.eventDate)).toLocaleDateString()}
              {isPastEvent && ' (ended)'}
            </span>
          </div>
          <div className="flex items-center text-gray-600">
            ...
            <span>
              ...
              {!isPastEvent && availability.validOffersCount > 0 && (
                <span className="text-amber-600 text-sm ml-2">
                  {availability.validOffersCount} {availability.validOffersCount === 1 ? 'person' : 'people'} trying to buy
                </span>
              )}
            </span>
          </div>
